...
/// Must not run at library load: [tz.initializeTimeZones] is called from [main] first.
tz.Location _tzIstanbul() => tz.getLocation('Europe/Istanbul');

tz.TZDateTime _tzNowIstanbul() => tz.TZDateTime.now(_tzIstanbul());

Methods methods = Methods();

/// İstanbul saati. `main` içinde `tz.initializeTimeZones()` çağrıldıktan sonra kullanılmalıdır
/// (getter olduğu için import anında çalışmaz).
tz.TZDateTime get now => _tzNowIstanbul();
...
class NotificationService {
  ...
  Future<void> scheduleWeeklyNotification(int id, String title, String body,
    ...
    final now = _tzNowIstanbul();
    final istanbul = _tzIstanbul();
  ...
  Future<void> scheduleAttendance() async {
    await scheduleAttendanceNotification(2000, "Devamsızlık Bildirimi",
        "Günlük devamsızlık kaydınızı girmeyi unutmayın.", 22, 30);
  }
